...
用法（本仓库是贴图源仓库，无游戏数据，需用 --data-dir 指向 CCB 的 data/json）：

  # 1) 先 compose 出成品（含 tile_config.json）
  python3 tools/gfx_tools/compose.py . ../composed_undead

  # 2) 列出贴图包已有的全部 id
  python3 tools/gfx_tools/list_tileset_ids.py ../composed_undead > tileset_ids.csv

  # 3) 生成多工作表追踪表，--data-dir 指向 CCB 游戏数据
